...
def chatsearch(request):
    ...
    if search1:
        search2 = Message.objects.filter(Q(student__user__first_name__icontains=search1) | Q(student__sid__icontains=search1) | Q(student__user__last_name__icontains=search1) | Q(msg__icontains=search1) ).order_by('-date').select( 'student' )
